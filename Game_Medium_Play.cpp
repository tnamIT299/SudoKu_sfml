...
#pragma region SetInitGrid
	...
	for (int i = 0; i < ISudokuMapGen::MaxSize; ++i) {
		...
		for (int j = 0; j < ISudokuMapGen::MaxSize; ++j) {
			...
			auto boxVal = this->_medium_map.gameMap[i][j];           // load values from gridMap 
...
void Game_Medium_Play::HoverCheck(sf::RectangleShape* button, sf::Text* buttonText) {
...
void Game_Medium_Play::OnFocusEvent() {
	...
	if (this->IsMouseOverButton(this->_submitButton)) {
		...
		int rol, col;
		if (this->_medium_map.FindUnassignedLocation(this->_medium_map.gameMap, rol, col))
		{
...
void Game_Medium_Play::HandleEvents(sf::Event* event) {
	...
	if (this->medium_play == true && this->_aiDone != true && this->_initAi) {

		auto check = this->AISolve(this->_medium_map.gameMap);
		this->_aiDone = true;
...
bool Game_Medium_Play::AISolve(int grid[ISudokuMapGen::MaxSize][ISudokuMapGen::MaxSize])
{
	int row, col;

	if (!this->_medium_map.FindUnassignedLocation(grid, row, col))
		return true;

	for (int value = 1; value <= ISudokuMapGen::MaxSize; value++)
	{

		if (this->_medium_map.isSafe(grid, row, col, value))
		{
			grid[row][col] = value;
...
void Game_Medium_Play::TimePlay() {
	//TIME
	sf::Font font;
