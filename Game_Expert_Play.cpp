...
#pragma region SetInitGrid
	...
	for (int i = 0; i < SudokuMapGen::MaxSize; ++i) {
		...
		for (int j = 0; j < SudokuMapGen::MaxSize; ++j) {
			...
			auto boxVal = this->_expert_map.gameMap[i][j];           // load values from gridMap 
...
void Game_Expert_Play::Display() {
	...
	for (int i = 0; i < SudokuMapGen::MaxSize; ++i) {
		...
		for (int j = 0; j < SudokuMapGen::MaxSize; ++j) {
			...
			if (this->_expert_map.gameMap[i][j] == 0)
				continue;
...
void Game_Expert_Play::MouseMoveTigger() {
...
void Game_Expert_Play::HoverCheck(sf::RectangleShape* button, sf::Text* buttonText) {
...
void Game_Expert_Play::OnFocusEvent() {
	if (this->IsMouseOverButton(this->_BackButton)) {
		...
		for (int j = 0; j < SudokuMapGen::MaxSize; ++j) {
			...
			if (this->IsMouseOverButton(this->_gameGridMap[i][j])) {
				...
				if (this->_expert_map.gameMap[i][j] == 0)
				{
					...
					this->_expert_map.gameMap[i][j] = this->_selectedNumber;
	...
	if (this->IsMouseOverButton(this->_submitButton)) {
		...
		int rol, col;
		if (this->_expert_map.FindUnassignedLocation(this->_expert_map.gameMap, rol, col))
		{
			...
			if (this->_checkErr == false) {
				...
				for (int i = 0; i < SudokuMapGen::MaxSize; ++i) {
					for (int j = 0; j < SudokuMapGen::MaxSize; ++j) {
						...
						auto value = this->_expert_map.gameMap[i][j];

						if (this->_expert_map.isSafe(this->_expert_map.gameMap, i, j, value) == false)
						{
							this->_checkErr = true;
	...
	if (this->IsMouseOverButton(this->_undoButton)) {
		...
		if (this->_selections.size() > 0) {
			...
			this->_expert_map.gameMap[field->row][field->col] = 0;
...
void Game_Expert_Play::HandleEvents(sf::Event* event) {
	...
	if (this->expert_play == true && this->_aiDone != true && this->_initAi) {

		auto check = this->AISolve(this->_expert_map.gameMap);
		this->_aiDone = true;
...
bool Game_Expert_Play::AISolve(int grid[SudokuMapExpert::MaxSize][SudokuMapExpert::MaxSize])
{
	int row, col;

	if (!this->_expert_map.FindUnassignedLocation(grid, row, col))
		return true;

	for (int value = 1; value <= SudokuMapExpert::MaxSize; value++)
	{

		if (this->_expert_map.isSafe(grid, row, col, value))
		{
			grid[row][col] = value;
...
void Game_Expert_Play::TimePlay() {
	//TIME
	sf::Font font;
